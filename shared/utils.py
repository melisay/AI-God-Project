...
# Ensure local_version/ can be imported
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "local_version")))

from tts import generate_tts

from config.config import VOICE_NIKKI, VOICE_TOM, current_voice
...
def switch_voice(user_input):
    """Handles switching AI voices dynamically"""
    global current_voice
    if "major tom" in user_input or "switch to major tom" in user_input:
        current_voice = VOICE_TOM
        generate_tts("Voice switched to Major Tom. Ready for lift-off!", "voice_switch.mp3")
        return True
    elif "nikki" in user_input or "switch to nikki" in user_input:
        current_voice = VOICE_NIKKI
        generate_tts("Voice switched to Nikki. Sassy mode activated!", "voice_switch.mp3")
        return True
    return False

def handle_song_request():
    """Handles song requests and plays a random song response"""
    song_response = random.choice(SONG_RESPONSES)
    generate_tts(song_response, "song.mp3")

def handle_impression():
    """Handles impressions and plays a random impression response"""
    impression_response = random.choice(IMPRESSION_RESPONSES)
    generate_tts(impression_response, "impression.mp3")
